...
function Product({uid, name, price, imageMarket, imageProduct, send, update}) {
    ...
    const Delete = async () =>{
        ...
        {
            update(Actions.PREDELETE);
            update(Actions.POSTDELETE);
            handleClose();
        });
    }
    ...
    return(
        ...
        <img src={"/".concat(imageMarket)+'.png'} width="100 px" height="100px" onClick={() => send({uid, name, price, imageMarket, imageProduct})}></img>
        <img className="w-20 card-img-top" src={"/".concat(imageProduct)+'.png'} alt="Product"></img>
        {!edit && 
                <div className="card-body" onClick={() => send({uid, name, price, imageMarket, imageProduct})}>
                <h5 className="card-title text-center">{name}</h5>
                <h5 className="card-title">Kaina:{price} €</h5>
              </div>}
        {edit && 
            <form onSubmit={SaveEditing}>
            <div className="card-body">
            <h5 className="card-title text-center">Pavadinimas:<input type="text" defaultValue={name}></input></h5>
            <h5 className="card-title text-center">Kaina, €:<input type="text" defaultValue={price}></input></h5>
